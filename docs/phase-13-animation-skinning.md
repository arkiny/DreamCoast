# Phase 13 — 스켈레탈 애니메이션 + GPU 스키닝/스킨 캐시 (세부 계획)
...
엔진**이라는 위상([phase-12-scene-graph.md](phase-12-scene-graph.md))에서 **스켈레탈 애니메이션**은
...
하고, 모든 소비처가 그 **단일 버퍼**를 읽게 한다(레퍼런스 엔진의 *GPU Skin Cache*와 동일 동기). 이것이 레이트레이싱
...
- `player.rs` — 재생 상태(`time`, `speed`, `looping`), `advance(dt)`. (Phase 12 도입 시 `AnimationPlayer`
  ECS 컴포넌트로 승격 — 아래 Phase 12 시너지 참조.)
...
- `SkinInfluence { joints: [u16; 4], weights: [u8; 4] /* unorm */ }` = **12B**(또는 가중치 f32×4 = 24B).
  스킨 캐시 **컴퓨트만** 읽는다. → `VertexLayout` 확장 불필요(PSO 처닝 0), 레퍼런스 엔진의 *separate skin weight
...
- **Phase 8 (HW RT) — 완료.** Stage D(스킨된 BLAS/TLAS)만 의존. Stage A–C는 RT 없이 래스터 경로로
  독립 출하 가능.
- **Phase 12 (씬 그래프) — 시너지, 하드 의존 아님.** Stage A–C는 **단일 스킨 메시 인스턴스**를 샌드박스
  하드코딩 씬에 추가해 독립 검증 가능. 다중 인스턴스 저작·재생 상태는 Phase 12 ECS의
  `AnimationPlayer`/`SkinnedMesh` 컴포넌트로 자연 승격(둘 중 어느 Phase가 먼저든 무관, 교차 참조만 기록).
- **`crates/scene`의 `Transform`** 표현 공유(중복 정의 금지). Phase 12 미구현 시 `anim`이 `core`에 정의
  → Phase 12이 재사용.
...
- **수정** `docs/ROADMAP.md` — Phase 13 추가, 워크스페이스 구조에 `crates/anim`, 기술 스택에 FBX 임포터.
...
- **Phase 12 순서:** 재생 상태/다중 인스턴스는 ECS 컴포넌트가 자연스럽지만 하드 의존 아님 — 단일 인스턴스로
