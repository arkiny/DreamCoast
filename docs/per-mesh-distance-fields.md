...
- 출력은 현재와 동일한 R32F 클립맵 거리 볼륨 → **거리 march 셰이더(`clipmap.slang cm_geo_*`) 무변경.**
  갤러리는 합성 결과가 기존 fused 베이크와 **수치 동일**(단일 메시·아이덴티티 = 동일 필드) → 바이트 동일.
...
- **S1 — 합성기(거리)**(`apps/sandbox`, 신규 `compose.rs`): per-mesh DF들 → 클립맵 **거리** 볼륨 +
  **nearest-object-id 볼륨** 합성(오브젝트 AABB 그리드 가속). 갤러리·`sponza_intel`에서 **거리 합성
  결과 vs 기존 fused 베이크 바이트 동일**(또는 ≤허용 FP) 검증. 아직 fused 경로와 병행(플래그).
