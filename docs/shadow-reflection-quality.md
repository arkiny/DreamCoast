...
## Phase 4 — 반사/GI 속 지면이 객체 색으로 물드는 문제 ✅ (범용 수정)
사용자 관찰: 스피어 반사에서 아보카도 밑/큐브 밑 지면 색이 다름(객체 색 침범). 두 원인:
1. **저해상 albedo 볼륨 + 해석적 지면**: 지면은 볼륨에 없어 `albedo_at(지면점)`이 **가장 가까운 객체 색**(큐브=빨강/아보카도=초록)을
   반환 → 재조명된 지면 반사가 객체 색으로 물듦(지배적, 넓은 색 밴드). `P11_GDF_COLOR=0`로 사라짐을 확인.
2. **surface cache 침범**: `sample_surface_cache`의 `dist > c0.w`(trace_depth=객체 두께) 허용오차가 너무 커서,
   지면에 앉은 큐브의 윗면 카드(법선 위)가 바로 밑 지면점과 매칭 → 지면이 큐브 캐시 색을 받음(블록 아티팩트).
**범용 수정**(원포인트 하드코딩 회피):
- **지면 머티리얼 단일 진실 공급원** `GROUND_ALBEDO`(main.rs 상수) → gbuffer 지면 드로우 + gdf_reflect + gdf_gi에 전달.
  트레이스가 이미 구분하는 지면 hit(`(p.y-GROUND_Y) < geo_inside(p)`)에 이 머티리얼 사용. 매직넘버 제거, 드리프트 방지.
- cache 허용오차를 trace_depth → **텍셀 기반**(`(|ua|+|va|)/tile*4`)으로 강화: 질의점이 캡처면 위(몇 텍셀)일 때만 매칭.
