...
- 환경 소스: **절차적 스카이**(`sky.slang`) — 추후 언리얼식 **Atmospheric Sky**로 교체 가능하도록 sky 셰이더만 분리.
...
### M7 — 마무리
- ImGui 디버그 뷰(G-buffer/shadow/직접광/IBL 분리), 라이트/노출/IBL 토글.
- 본 문서 검증/한계 채우고 ROADMAP Phase 6 ✅.
...
- Vulkan 검증 클린(`VK_LOADER_LAYERS_DISABLE=~implicit~`). RenderDoc/PIX로 G-buffer 확인.
