...
- **V1 — 정적 VSM 코어**: 마킹→할당→래스터→샘플(캐시 없음, 매 프레임 전체 재렌더).
  ...
  **상태: 착지(2026-08-02).** `VSM=1` seam으로 동작 — vsm_common/pages/depth.slang +
  `vsm.rs` + `begin_rendering_empty`(3 백엔드 RHI) + pbr `vsm_sun_shadow`(레벨 폴백 워크,
  페이지 경계 안전 3×3 리시버-플레인 PCF, per-tap 페이지 변환). 던전 실측: 페이지
  206–227/512 할당·오버플로 0, 캐릭터/벽 그림자 밀착, 갤러리 앵커 관성(런투런 플로어),
  런투런 0.34 = 씬의 wall-clock 플로어 이내, DX≡VK 0.97(플로어 ~0.55의 2배 이내 —
  시간-콘텐츠 결정화(§0 캡처 경로 wall-clock 공백) 전까지 엄격 게이트 유보).
  브링업에서 잡은 것: (1) **consts 행렬 로드 전치 버그** — `to_cols_array`를 행으로
  재조립하면 마킹·샘플링은 서로 일치하지만 래스터(푸시 cbuffer 규약)와 어긋나 맵드
  페이지 위 프래그먼트가 12%뿐이었다(교훈: 수동 ByteAddressBuffer 행렬 로드는 반드시
  transpose — vsm_common.slang 주석 계약화). (2) 수직 벽 grazing 아크네 → bias
  2텍셀 + 슬로프 클램프 0.1(CSM의 0.05 대비 — mm 텍셀에선 평면 추종을 더 길게).
  V1.5 잔여: 레벨별 캐스터 컬링(현재 전 캐스터×6레벨), PROFILE_GPU에 vsm 패스명 등록,
  풀 통계 상시 DIAG화. 스킨/모프 캐스터 변형은 컴파일만 검증(던전은 리지드).
