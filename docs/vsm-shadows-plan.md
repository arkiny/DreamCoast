...
- **V2 — 페이지 캐싱 + 클립맵 스크롤**: 2슬라이스 풀, 수명주기 4리스트(첫 컷은
  ...
  **상태: 착지(2026-08-02).** 단일 슬라이스 구현(2슬라이스는 V4 레버로 이월): 물리
  페이지 메타(주소/최종요청프레임/valid)에서 매 프레임 테이블 재구성 + CPU 스크롤
  오프셋 적용, age 120 강퇴, 레벨별 핀-Z 가드밴드, 태양 방향 비트-키 무효화. 실측:
  정지 정상상태 **재렌더 0페이지**, 이미지 V1±플로어. 브링업 교훈: **영속 캐시
  버퍼(메타/프리리스트)는 반드시 제로-초기화**(device-local 미정의 초기값이 유효
  플래그/카운트로 읽혀 물리 페이지 이중 배정 — 정지 씬은 테이블 경유 자기일관성
  때문에 우연히 동작하고 스크롤에서만 터지는 함정).
- **V3 — 동적 무효화** (지시의 본체): CPU 수집기 + GPU `InvalidateInstancePages` +
  ...
  **상태: 착지(2026-08-02).** CPU가 캐스터 transform 비트를 프레임 diff → 변경 시
  **구+신 풋프린트 구체**(UE 이중 풋프린트 규칙), 변형 캐스터(skin/morph/vcache)는
  매 프레임, **릭 파츠 구체는 그리디 병합**(~릭당 1개 — 미병합 시 7캐릭터가 64예산
  초과로 전체 무효화 강등). csUpdate가 구체를 레벨 텍셀 공간에 투영해 겹친 캐시
  페이지만 clear+RENDER. 실측: 정지 0, 솔로 질주 6페이지/프레임, 7캐릭터
  84페이지/프레임·오버플로 0·잔상 0. 할당기 2건 근본 수정: ① 프리리스트를 **매 프레임
  메타로부터 재구성**(csCompact = UE PackAvailablePages 형태) + 단조 pop 커서 —
  영속 스택의 decrement-undo 팝은 경합에서 팬텀 팝으로 물리 페이지 이중 배정(87
  오버플로/프리 0으로 붕괴 실측); ② 레벨 선택 1스텝 완화(floor(log2 d), 텍셀 ≈
  스크린픽셀 1.2×) — 과세밀 선택은 레벨0 마킹만으로 512 풀을 초과. 상시 애니 릭
  84페이지/프레임의 절감(UE 정적/동적 2슬라이스)은 V4 레버.
  **던전 앱 기본 = VSM=1 (CSM deprecated for game content)** — CSM/레거시 경로는
  앵커 씬 + `VSM=0` 폴백 seam으로 유지.
