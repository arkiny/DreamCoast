...
- **비동기 PSO 컴파일(Phase 12 M5.2 결속):** 청크 스트림-인 시 새 머티리얼이 **런타임 파이프라인(PSO/
  VkPipeline) 생성**을 유발 → 프레임 히치. [phase-12-m5-async-pipeline.md](phase-12-m5-async-pipeline.md)
  **M5.1**(준비성 슬롯 + 지연 드로우: 미준비 PSO 패스는 준비될 때까지 스킵)이 토대를 깔고, **M5.2**(배경
  스레드 PSO 컴파일, 백엔드 `Send`/`Sync` 감사 후)가 생성 비용을 프레임 시간에서 완전히 제거한다. 스트리밍이
  M5.2의 **실제 수요처** — 현 정적 씬은 시작 비용이 작아(~1s) M5.2 이득이 미미하므로, 스트리밍 도입 시
  착수한다. 청크 로드 예산(프레임당 1청크)과 PSO 컴파일 예산을 함께 조율.
