...
## F2 — mesh SDF 확장성 (실측 재스코프: 브릭 → 비등방 dims + f16) ✅ S2a/S2b 랜딩

**결과 (2026-07-13,** [phase-f2-mesh-sdf-scalability-plan.md](phase-f2-mesh-sdf-scalability-plan.md)**):**
S1 프리미티브 실측이 브릭 전제를 반박(현행 해상도 점유 74–85% → 브릭 ×1.5 손해; 블랭킷 해상 상승 GB급)
→ 재스코프. 랜딩: **S2b f16 아틀라스**(`P11_ATLAS_F16`, rhi `R16Float` 3백엔드) + **S2a 비등방 per-axis
dims**(`SdfVolume.dims[3]`, 얇은 축 과해상 제거). 아틀라스 71.0 → **28.6 MB(×0.40)**, per-axis 캡 48,
PT budget PASS(interior −0.20 개선). **브릭(원 S2)은 보류** — 재평가 조건: 점유 <40%(도시급 씬/고해상).
mip 체인(cone LOD)은 별도 페이즈로 분리(F4와 우선순위 비교). 아래 원문은 이력 보존.

**문제(원문):** `crates/asset/src/sdf_atlas.rs`는 dense `dim³` 타일(캡 32³)만 — sparse/brick/mip 없음. 확장성 상한.
