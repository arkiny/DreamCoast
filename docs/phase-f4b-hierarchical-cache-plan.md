...
> 상태: **완료 — 증분 1·2·4·5·6 랜딩(전 게이트 그린), 증분 3(기본 ON)은 공식 측정으로 보류
> 판정, 증분 7 미발동** (2026-07-15). 실행 기록은 §2, 기본 ON 판정은 §5, 페이즈 종합은 §13.
> 상위 [gi-fidelity-roadmap.md](gi-fidelity-roadmap.md) §F4,
...
**실행 기록(2026-07-14~15)**: 실제 랜딩 순서는 1(occ `310d35a`) → 2(인터리브 `297efe3`) →
4(반사 폴스루 `add89b9`) → 5(edge-fade `8a04c10`) → 3(기본 ON 판정) — 증분 3을 완성 스택
위의 단일 측정으로 미룬 것은 수동 sunlit 행이 +0.20 회귀를 보였고 그 원인(경계 seam)이 증분
5의 수리 대상이었기 때문(§5의 판정 기록 참조). 중간에 측정 인프라 개선 2건이 병렬 세션에서
합류: **AE FIXED_DT**(스크린샷 모드 결정론, `fbf2c37`) 덕에 PT budget이 소수 3자리
재현(27.952/31.608)되어 sunlit 28.25/interior 31.91로 하향 재기준선(`f3cf266`), 콘텐츠 골든
2종이 strict SHA로 복원.
...
> **판정 기록(2026-07-15, 공식 러너)**: 완성 스택(occ+인터리브+α0.2+반사FT+fade, tint 0.15)의
> 기본 ON 공식 측정 — **interior 31.627(≤31.91 PASS, coarse 31.608 대비 +0.02 중립)** /
> **sunlit 29.080(>28.25 FAIL, coarse 27.952 대비 +1.13)**. 단 물리 지표는 개선: sunlit
> lit_mean 비율 1.078→**1.008**(F6C의 "lit 갭" 실질 소거), interior 1.33→1.18, EV11 깊은
> 크롭 40.9→35.4(PT 방향), 크롭 B−R −0.60=PT. **함정 갱신 — 수동 행의 스테일-미터링**: PT
> 캡처는 raster와 같은 adapted-exposure 버퍼로 노출되므로(golden-image.py:89-96) 기본값
> 변경처럼 raster HDR을 바꾸는 실험에서 구-설정 PT 재사용은 무효(수동 sunlit 27.970 vs 공식
> 29.080). knob 행간 비교도 raster+PT를 **같은 설정으로 쌍 캡처**해야 유효.
> **수렴은 원인이 아님(기각 실측)**: 같은-설정 쌍 384 워밍업에서 sunlit 30.358로 더 악화,
> lit 비율 0.977(**언더슈트 역전** — coarse +7.8% 과밝음 → 수렴 fine −2.3% 과어둠). 이에
> 따라 "워밍업 부스트 스케줄" 아이디어도 기각.
>
> **최종 판정 — 기본 ON 보류(opt-in 유지)**: 공식 tint 프로브 t0.2 = sunlit 28.179 PASS /
> interior 32.041 FAIL. 두 게이트의 선형 창 = sunlit t≥0.196 ∩ interior t≤0.184 → **공집합**
> (중간점 t0.19는 양쪽 FAIL 예측). t0.2의 sunlit PASS도 lit 비율 1.082 복귀(중립 필이 fine의
> 충실도 개선을 도로 덮은 것)라 승리가 아니다. 구조 해석: fine 필드는 coarse의 **바이어스
> 잔차**(균일 과밝음 — masked_avg가 관대)를 **산포 잔차**(평균은 정합, 분산 증가 — masked_avg가
> 가혹)로 바꾼다. 랜딩: 기본 플립 revert, opt-in 사용자용 fine-조건부 tint 0.15만 유지.
> **후속 리드**(이 페이즈 스코프 밖, 순서 제안): ① 볼륨-조건부 mb 부스트 프로브(tint와 달리
> albedo-가중이라 공간 시그니처 상이 — 언더슈트 에너지의 다른 회수 경로) ② fine box half 공식
> 스윕(5 m — sunlit 산포 면적 축소) ③ **메트릭 논의(F6 트랙)**: masked_avg와 lit_mean 비율이
> 정반대 판정을 내는 사례 확보 — 게이트 메트릭에 바이어스/산포 분해 추가 검토.
...
> **증분 6 검증 기록(2026-07-15)**: 정적 불변 — CLIP=1 interior 캡처가 재중심 랜딩 전과
> **바이트 동일(diff 0.000/max 0)**, 공식 러너·풀 매니페스트 불변. 모션 스팟체크 —
> `CAM_EYE_END=-8,2,0`(6 m 병진, 24프레임, 데드존 3.6 m 교차 강제)에서 f2f 프로파일이
> coarse 기준선과 페어 단위 일치(mean 21.65 vs 21.55, 최대 델타 +0.83 = 패럴랙스 ~20의
> 4% — 재수렴 윈도 전환이 fade+coarse-파크로 흡수). 스파이크/펌핑 없음.
...
> **판정(2026-07-15): 미발동.** 착수 조건의 전제(기본 스택에 fine 편입)가 §5의 보류 판정으로
> 성립하지 않는다 — 레벨을 더 쌓기 전에 §5의 후속 리드(산포 잔차의 근본: mb 프로브·box half·
> 메트릭 분해)가 선행이다. 캐스케이드는 그 결론이 요구할 때 별도 페이즈로.

## 13. 페이즈 종합 (2026-07-15, 전부 Metal 공식 프로토콜)

| 커밋 | 증분 | 게이트 결과 |
|---|---|---|
| `87b30b9` | 계획서 | — |
| `dc62d20` | 스테일 콘텐츠 골든 재시드 | 사전 존재 비결정성 노출(후속 태스크 발행) |
| `310d35a` | ① occ-가중 fine 소비 포팅 | CLIP=0 불변 ALL PASS·색 앵커 개선(B−R −1.58→−1.35) |
| `fbf2c37`+`f3cf266` | (병렬 세션) AE FIXED_DT + budget 래칫 | PT budget 소수3자리 재현·**28.25/31.91 하향**·콘텐츠 골든 strict SHA 복원 |
| `297efe3` | ② 레벨-인터리브 슬랩 + α 레버 | 비용 +1.9%(목표 ≤10%)·셔머 0.0403/0.0457·CLIP=0 비트동일 |
| `add89b9` | ④ 반사 fine 폴스루 | 게이트-중립(일관성 수리)·CLIP=0 비트동일 |
| `8a04c10` | ⑤ edge-fade | **sunlit seam 비용 회수**(28.153→27.970)·interior 31.410 |
| `ce65469` | ③ 기본 ON 판정 | **보류**(tint 창 공집합; lit 비율 1.078→1.008 개선은 실재 — §5) |
| (본 커밋) | ⑥ 카메라 재중심 | 정적 바이트 동일·모션 델타 ≤+0.83(패럴랙스의 4%) |

기본 경로(coarse)는 페이즈 전후 비트-동일(27.952/31.608, 갤러리 `65d04ce…` 불변). fine은
`P_GI_VOL_CLIP=1` opt-in으로 완전한 스택(occ·인터리브 평탄 비용·α0.2·반사 폴스루·fade 0.15·
tint 0.15·재중심)을 제공한다. DX≡VK Windows 배치(동결)에 추가: gi_volume 인터리브/y-오프셋,
reflect flip_y bits15..26 디코드, CLIP=1 콘텐츠 캡처, CAM_EYE_END 경로.
