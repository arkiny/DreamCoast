...
#### C8b — 서피스 캐시 라디언스 (풀, 정확 = 설계 포크 B) — **카드/서피스 캐시 채택**
**사용자 결정(2026-06-26): "가장 상용 엔진에 가까운" 표현 = 메시 카드 + 서피스 캐시 아틀라스**
(오리엔티드 쿼드 그리드). per-voxel 라디언스 그리드(복셀 GI 계열, C8a 인프라 재사용·저위험)도 후보였으나
상용 충실도 우선. 카드: 임의 메시에 좋은 UV가 없으므로 **오브젝트 AABB 6면 박스 프로젝션 카드**
...
- **정직한 결과(rt-compare):** **서피스 캐시는 이 씬에서 PT 잔차를 줄이지 못함.** SW-RT 반사 2.58→2.92/ch
  ...
  캐시는 상용 엔진식 멀티바운스 아키텍처 + 컬러 캐시 라디언스를 제공하나 **opt-in 유지**(기본 무회귀).
...
#### C8d — 풀-res 미러 SSR 복원 = 정확한 반사 컬러 (상용 엔진 정렬, 2026-06-26)
사용자 지적 = 반사 컬러가 PT와 다름(크롬에 반사된 아보카도 밑 초록 줄, 바닥이 길게 늘어짐) + 반사
maxroughness 별도 스레시홀드 요청 + (밉-피라미드/SSR-에너지 리커버리가 해결하면 구현).
**상용 엔진 반사 파이프라인 참조:** 스토캐스틱 GGX 레이 → **스크린 트레이스 먼저(정확한 온스크린 컬러)** →
실패 시 월드/SDF + **서피스 캐시 라디언스**(per-ray 재조명 아님) 폴백 → 디노이즈. **최대 러프니스 임계**
위는 반사 트레이스 생략·GI/라디언스캐시로 대체. **핵심: 스크린 트레이스가 *주(정확)*, 월드/SDF는
...
한계. **완전 해결 = Hi-Z 마치 + thickness 정교화, 또는 서피스 캐시를 히트 라디언스로(오프스크린/
그레이징 정확 컬러).** 밉-피라미드는 블러만(컬러 미해결) → SSR-에너지 리커버리가 정답이었음.
- **NEXT 후보:** 그레이징 초록줄 → Hi-Z SSR 마치/thickness, 또는 서피스 캐시(C8b) 히트 라디언스 결합
  (서피스 캐시 정렬, 오프스크린 컬러 정확). 풀-res 미러 2.58 잔차 회복. GDF 재조명 포인트라이트.

#### C8e — 상용 엔진 반사 합성 벤치마크 + 구리구 하이라이트 blow-out 수정 (2026-06-26)
사용자 요청 = 상용 엔진(벤치마크 대상)이 SSR/SW-Reflection/스카이라이트를 어떻게 칠하는지 상용 엔진
셰이더 소스 참조(SSR은 스토캐스틱 유지) + 구리구 좌측 하얗게 타버린 구간 원인·수정.

**상용 엔진 반사 셰이더 소스 참조:**
- **반사 환경 합성 셰이더 = 합성 모델.** 핵심:
  ...
  리플렉션캡처(SW-reflection)+스카이라이트가 채움**. 순서 = **SSR(정확,위) → 캡처/서피스 캐시 → 스카이라이트
  (전역 폴백,아래)**. 마지막에 `Color.rgb *= EnvBRDF(SpecularColor, Roughness, NoV)` 1회.
- **러프니스 = 프리필터 밉.** 캡처·스카이라이트는 러프니스→밉 매핑으로 샘플(에너지 보존 밉=소프트
  하이라이트). 스크린스페이스 블러 아님.
- **반사 합성 알파 = `saturate((max_roughness - Roughness)·inv_fade_length)`** — 우리 C8d `max_roughness`
  스레시홀드와 동일(우린 smoothstep). 위는 트레이스 생략→캡처/스카이라이트(밉) 폴백.
- **스토캐스틱 SSR 유지 가능** — 스토캐스틱 GGX + ratio-estimator는
  `P11_SSR_STOCHASTIC` 경로에 보존.
- **우리 정렬 평가:** 단일 GDF 폴백에선 우리 `lerp(gdf, ssr, ssr.a·trust)` ≡ under-연산자. EnvBRDF는
  pbr.slang에서 1회 적용 ✓. **갭 = 에너지보존 프리필터 밉** vs 우리 게더 박스블러 = blow-out 원인.
...
Poisson 패턴=스페클, 펼쳐서 flat-white로 blow-out. (= 레퍼런스 엔진가 에너지보존 밉으로 푸는 부분.) **수정 = 프리필터
...
미반영) — 완전 정합은 에너지보존 밉-피라미드 프리필터(상용 엔진 정공법).
...
- **해상도 확인 결과 = 메인 RT 안 줄어듦.** 코드 확인: 전 풀스크린 타깃(g_albedo/normal/material/
  ...
  **half-res(`hcw/hch`)는 스토캐스틱 SSR accum 버퍼 전용**(C8d 기본 off = `P11_SSR_STOCHASTIC`만). 즉
  ...
  반사 게더 무관) — 해상도 저하 아님.
- **NEXT 후보(반사 트랙):** ① **에너지보존 밉-피라미드 프리필터**(레퍼런스식, GDF 반사 다운샘플→러프니스 LOD)
  = 게더 대체, 큰블러 저비용 + blow-out 근본해결. ② 그레이징 초록줄 → Hi-Z SSR/thickness. ③ 서피스
  캐시(C8b) 히트 라디언스(서피스 캐시, 오프스크린 컬러). ④ 알파-under 멀티레이어 합성(스카이라이트 분리).

#### C8g — 서피스 캐시를 GDF 반사 히트 라디언스로 (그레이징 초록줄 완화, 2026-06-26)
...
잔존(컬러만 정확해짐). **NEXT: 밉-피라미드 프리필터, 또는 캐시 해상도↑/카드 시임 개선.**

#### C8h — 반사 트랙 마무리: SSR 진단·캐시해상도 검증·밉-피라미드 프리필터 (2026-06-26)
사용자 "다 한번에" → 세 항목 진행. **① SSR 근본원인 = 버그 아님(씬 한계, 코드 무변경):** hit/miss 디버그
viz(blue=오프스크린, red=노크로싱, white=hit)로 확인 — **구의 반사 레이가 대부분 화면을 벗어남**(작은
오브젝트+큰 하늘+카메라 각도 = 반사가 하늘/오프스크린 지배). max_dist×6도 더 많은 레이가 오프스크린만 됨
(레인지 한계 아님). 즉 SSR은 온스크린 림만 정상 기여, GDF가 나머지 채움 = 정상. 반사 품질은 GDF-bound.
**② 캐시 해상도 bump(CARD_TILE 32→64) = 무효과·되돌림:** PT 잔차 3.44→3.443(동일), 시각 동일. 블롭
형상(SDF)이 지배라 캐시 컬러 샤프니스는 무관 → 비용만↑(아틀라스 4×) → 32로 복원.
**③ 밉-피라미드 프리필터 ✅(게더 박스블러 대체, 레퍼런스식):** `reflect_downsample.slang`(4× 박스 다운샘플,
4 bilinear 탭) 체이닝으로 GDF 반사 밉(1/4·1/16·1/64) 생성 → `reflect_composite`가 러프니스 LOD로
트라이리니어 샘플(`lod=saturate((rough-0.04)/0.56)·3`). 에너지보존 평균 = 밝은 픽셀이 소프트 하이라이트로
적분(게더의 점광원 복제 문제 없음). `ReflectSystem::record_reflect_mips`(3패스), composite push 게더
파라미터(depth/blur_scale/depth_reject) 제거하고 밉 인덱스로 교체(48B 유지). **결과:** 코퍼 글로시 깔끔
(`docs/images/hybrid-vs-pt.png`), hybrid-vs-PT **3.44→3.41/ch**(>32 2.04%), **VK VUID0/DX clean,
VK≡DX 0.001, 레거시 무회귀.** **비용 대폭↓: reflect_composite 0.206→0.037ms + 다운샘플 3패스 0.022ms =
0.059ms(게더 0.206 대비 ~3.5배 빠름)** — 레퍼런스식 밉이 게더보다 싸고 깔끔. **반사 트랙 현황:** 컬러·러프니스·
이중스펙·blow-out 다 처리; 남은 본질 한계 = GDF 저해상 48³ SDF 블롭 형상(클립맵/고해상 SDF가 다음 레버,
B3 클립맵 계획 참조) + SSR 씬한계(온스크린 반사 적음).

#### C8j — 스토캐스틱 GGX GDF 반사 + 시공간 디노이즈 (구리구 과밝음 근본수정, 2026-06-26)
사용자 = 구리구 왼쪽 하얀색(러프 금속 과밝음) 상용 엔진 참조해 수정. **상용 엔진 반사 셰이더 소스 확인:**
러프 반사를 **이미지-스페이스 프리필터가 아니라 실제 GGX 로브 레이로 트레이스**
— VNDF importance-sample로 픽셀당 1 지터 레이(콘 각=1/PDF), 미러는 `reflect(V,N)`; 이후
스페이셜 resolve + 템포럴 디노이즈, **톤맵-스페이스 변환**
...
(이웃 픽셀의 서로 다른 GGX 레이 평균=분산↓) + 톤맵-스페이스 누적(톤맵가중). ping-pong
...
(gdf_reflect 0.15→0.50=GGX 레이 발산으로 마치 비코히어런트, temporal 0.35, composite 0.04) — 레퍼런스식 정공법
...
à-trous 강화) 또는 cone→GDF mip(레퍼런스식 저비용 와이드 콘); GDF 저해상 블롭은 여전히 B3 클립맵 트랙.**
