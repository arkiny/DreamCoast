...
## Stage 9 — grazing 바닥: 이방성 필터링 (기본 ON=16, 2026-06-28; 기본화 2026-07-13)
...
**DX≡VK 트레이드오프 (2026-07-13 기본화 결정)**: 이방성은 드라이버 의존 → **DX≡VK 0.427/ch (0.46% 픽셀 >8)**.
등방성 기본은 grazing 바닥을 줄무늬로 붕괴시켜 실사용 품질 결함이었으므로, 소유자 결정으로 **세 백엔드 모두
기본 `P_ANISO=16`**(올바른 프로덕션 기준). golden 앵커(gallery)도 이방성 출력으로 재베이스(SHA
`65d04ceca2c4…`, `P_ANISO=1`로 구 앵커 `af70c1a5…` byte-identical 복원 확인). CLAUDE.md 하드룰에
