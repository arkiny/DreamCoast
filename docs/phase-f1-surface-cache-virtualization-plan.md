...
### Stage 3 완료 — 검증 결과 ✅ 커밋 `d76f861`
`P11_CACHE_STREAM` opt-in(콘텐츠, 풀 함의). **설계(B) per-slot 오버라이트** 구현: `stream_residency`가 매
프레임 라이브 카메라로 target residency 재계산 → **순수 집합 차분**(evict/admit) → 승인 draw의 6카드
in-place host-write(`append_drawable_cards`) + `slot_dirty` → capture 패스가 dirty 카드만 재트레이스(부분
재캡처). LRU readback 불필요(set-diff로 충분), relight reset 생략(EMA 수렴). 그리드는 스트리밍 시 비활성.
**검증(Metal, release):** 갤러리 앵커 바이트 동일 / 정적 카메라=no-op(sane·결정론) / **동적(DIAG orbit+
CAPTURE_SEQ): 프레임당 1–5 슬롯 재소유, 170 resident 유지, 프레임 sane(0% black·무손상), 크래시 없음,
run-to-run 0.047/ch(노이즈 플로어)**. clippy/fmt clean. **후속: card_touched 미러-가시 방출·우선순위 방출·
grid-over-slots·freeze 재-arm 튜닝.**

### Stage 3 (원설계) — LRU 방출 + 데맨드 승인 + 부분 재캡처 (스트리밍 코어; 최대·최고위험 단계)
