...
**메커니즘(정련):** 프레임당(상각): (1) **CPU: 라이브 카메라로 target 레지던시 재계산**
(`select_card_residency(live_cam)`) → admit=target∖current, evict후보=current∖target. (2) `card_touched`
R프레임 늦게 fence-guarded readback → 방출 우선순위(min touched, tie=draw-index; 미러-활성 카드 보존).
(3) `slot_to_card` 갱신(방출 슬롯을 승인 카드에 재배정) + host write. (4) 승인 카드 **부분 재캡처**
(프레임당 K예산; capture-once 래치를 per-슬롯 dirty 마스크로 확장; `relayout_from_feedback`의 wait_idle
재캡처를 per-슬롯 dispatch로 축소) + 해당 슬롯 relight reset. (5) 방출 카드 → coarse-fallback(Stage 0으로
dense-lit). **within-budget/갤러리는 정적 all_resident**(풀 미사용).
...
**리스크:** async 3슬롯 ring vs 신규 레지던트 첫 relight 타이밍; 카드 그리드 재빌드(페이지 in/out); freeze
래치가 정적 카드 집합 가정 → 스트리밍 중 재캡처 시 epoch/freeze 재-arm 필요.
