...
#### C8c — stochastic SSR 후 PT 잔차 재측정 (정직한 회귀 발견, 2026-06-26)
...
→ **C8c 후속(다음 단계): reflect_composite/ssr_resolve 러프니스-aware 정교화** — 러프니스로 SSR↔GDF
블렌드(샤프 금속=GDF/미러 신뢰, 와이드=SSR 글로시) + resolve den=0 프레임 history 보존(감쇠 방지) +
커널 반경 러프니스 적응 + half-res 업샘플 디스오클루전. 회복 못 하면 stochastic 기본 재고(미러로 롤백).
