...
- **M4.4 — 퍼뮤테이션 매트릭스.** `Job`에 `variants`(이름 붙은 define-set) 선언 → **(잡 × 변형 × 타깃)**
  ...
  특수화로 경로 검증.
- **M4.5 — 빌드 병렬화. ✅ 완료(2026-06-27).** 캐시-미스 잡들의 `slangc` 서브프로세스를 워커 스레드풀
  (`available_parallelism`, 무의존 `std::thread` + 공유 큐)로 동시 실행. 2-패스: 사전 패스가 미스만 모아
  병렬 컴파일 → emit 루프는 동일 `cell_key`로 hit/miss 재판정 후 결과 소비(매니페스트 조인 후 기록).
  Metal RT-pipeline 분기는 순차 유지(macOS 전용). **측정:** 콜드 빌드 **56s→16s(~3.5×)**, 산출물
  **118/118 byte-동일**(병렬은 스케줄링만 변경), touch→0 compiled, 양 백엔드 DX≡VK 0.000/ch.
