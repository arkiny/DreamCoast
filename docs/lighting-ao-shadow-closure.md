...
## 5b. 웨이브 2 — gi_volume 수렴-프리즈 + mipgen 게이팅 (2026-07-25 후속, 랜딩)

동결 릴라이트 계약("settled 프레임은 스킵, 핑퐁 고정")을 볼륨으로 확장: 캐시 프리즈
래치(결정론적 lighting-epoch 지평) + fine 박스 steady + 정착 2사이클 완료 시 볼륨
업데이트 스킵(바인드만). 볼륨 전용 클럭(`gi_vol_clock`, 기록 프레임에만 전진)으로
슬랩 워크·방향셋 위상·핑퐁이 해동 시 정확히 이어짐. 리센터 arm은 즉시 해동(교착
불가). mipgen은 settled 동안 바인드-온리(밉0 불변 → 피라미드 유효). `P_GI_VOLUME_FREEZE=0` 탈출구.

| 상태 | DX door/nave/atrium | VK nave/atrium |
|---|---|---|
| 정지(수렴) | **8.7 / 11.7 / 10.4 ms (115/86/96fps)** | **10.7 / 9.3 ms (94/107fps)** |
| 이동(데드존 내 돌리, 62f 윈도우) | door 9.8 (102fps) / nave 12.7 (79fps) | — |

게이트: 플리커 래칫 유지(돌리 ROI 2.06/2.01 ≤ 2.2; 정지 0.171), 리센터-팝 프로브
(8m 워크 96f: max/avg 1.4 = 패럴랙스 클래스, 팝 없음), 갤러리 0.000(max4), door 정지
스틸 wave-1 대비 0.419/ch(>8: 0%) — 4-셋 리밋사이클의 위상 고정에 따른 미세 편이
(문서화된 의도), PT 게이트 기존 발산 레짐 내(±1).

## 6. 남긴 것 (정직한 잔여 목록)

1. **이동 중 지속 60fps의 구조적 마무리**: 데드존 내 이동은 79~102fps로 통과.
   리센터/스트림 더티 구간은 버스트 상각(리센터당 ~16 기록 프레임) — 장거리 연속
   이동에서 프레임 스파이크가 거슬리면 gi_volume 비동기 겹침(뷰-독립·핑퐁 구조상
   자연스러운 async 후보, Phase-E 후속)이 다음 레버. d3d12 배리어 배칭(32개/프레임),
   vgeo 각도별 비용(nave cut 3.1ms)도 후보.
