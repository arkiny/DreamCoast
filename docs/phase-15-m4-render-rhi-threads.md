...
멀티스레드로 가른다. **사용자 결정: 렌더그래프 스레드와 RHI 스레드를 분리한다** (레퍼런스식 Render thread ↔
RHI thread 모델):
...
- **옵션 B — RHI 커맨드-리스트 IR (레퍼런스 엔진 RHICommandList식, 권장).** record(렌더그래프) 스레드는 **백엔드
...
**결론: B3는 VK/DX 양쪽에서 정확.** 스레딩 버그·검증 에러 없음.

#### 별건(B3/B4 무관) 정리 (2026-06-29)
- **VK `fragmentStoresAndAtomics` 경고 — 해결.** auto-exposure가 deferred 라이팅 **fragment** 스테이지에서
  bindless RW storage buffer(`g.storage_buffers`, 노출값)를 읽어 VUID-RuntimeSpirv-NonWritable 발생.
  VK 디바이스에 `fragment_stores_and_atomics` 기능 활성화(이미 켜진 `vertex_pipeline_stores_and_atomics`와
  대칭)로 제거. 렌더 출력 불변(VK 캡처 해시 `06BDD797…` 동일).
- **D3D12 1-LSB 런-투-런 비결정성 — 국소화 + 무해 판정.** GDF SW-RT를 끄면(`P11_LEGACY_IBL=1`) D3D12도
  런-투-런 바이트 동일(결정적)이나, GDF가 활성이면 1-LSB(avg 0.000, max 1) 지터 발생. 개별 소비자
  (reflect/GI/AO/cache/firefly)를 하나씩 꺼도 남고 **전부 꺼야(=legacy) 사라짐** → 단일 패스 버그가 아니라
  GDF SW-RT 컴퓨트(픽셀별 march + 디노이저) 전반의 **부동소수 비결합성/D3D12 워프 스케줄링** 특성(VK는
  우연히 결정적). **시각 영향 0**(0.000/ch), 정확성 버그 아님. 검증 게이트는 이미 백엔드 자체 노이즈 플로어
  기준으로 처리하므로 추가 조치 불필요(진짜 bit-exact가 필요하면 결정적 리덕션 강제 = 비용↑, 보류).
