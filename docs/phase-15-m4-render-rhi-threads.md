...
### B1 — 커맨드 IR + 번역기 (foundation, 가산적·렌더러 무변경)
- `rhi`(또는 신규 모듈)에 `RhiCommand` 열거형 + `CommandList` + `Recorder`(append API) + `translate(&CommandList,
  &CommandBuffer, &Device)`(실제 백엔드 cmd로 재생). 우선 **핵심 서브셋**(begin/end render pass, bind
  graphics/compute pipeline, push constants, set viewport/scissor, bind vertex/index, draw/draw_indexed,
  dispatch, 배리어, copy)부터; 패스 마이그레이션하며 커맨드 추가.
- 검증: IR 라운드트립 단위 테스트(record→translate 호출 수/인자 일치). 렌더러 경로 무변경 = 무회귀.

### B2 — 기록을 IR로 라우팅 (캡처 바이트 동일)
- `RenderGraph` 패스 실행부 + 전 `record_*`(deferred/gdf/gi/reflect/rt/ibl/particle/cull/mesh/gui)를 `cmd`
  직접 호출 → `Recorder` append로 전환(또는 `Recorder`가 `cmd`를 감싸 양쪽 지원). 한 프레임을 IR로 모아
  곧바로 같은 스레드에서 translate → 기존 inline 프레임과 **캡처 바이트 동일**해야 함.
