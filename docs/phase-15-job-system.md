...
### M4 — (후속, 별도 Phase/마일스톤) 병렬 RHI 커맨드 기록
> **본 Phase 범위 밖.** 토대(M1–M3)가 검증된 뒤 착수. 렌더 그래프 패스를 여러 워커가 **세컨더리 커맨드
> 버퍼/번들**로 동시 기록 → 메인이 병합 submit. 양 백엔드(VK secondary CB / D3D12 bundle / Metal
> parallel encoder) 병렬 기록 검증이 무거워 의도적으로 분리. 별도 계획 문서로 분기.
