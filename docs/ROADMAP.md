...
│   ├── scene/          # 씬 그래프(변환 계층) + 레벨/월드(스트리밍) — RHI 비의존 (Phase 13)
...
변환 계층 씬 그래프(arena 트리, 로컬 TRS→월드 dirty 전파)와 선언적 레벨/스트리밍 월드를 구축한다.
씬 그래프가 래스터·HW RT(TLAS)·GPU 컬링에 per-instance 트랜스폼을 공급하는 단일 소스가 된다.
테스트는 보유 애셋(Avocado/BoomBox/Lantern)으로 구동. 전제: Phase 4(애셋)·5(렌더그래프)·8(RT).
...
- **직렬화**: 레벨/월드 데이터 모델은 처음부터 serde-ready(RON 텍스트 로드·저장) → 차후 Phase 12
  컨테이너로 쿡(위 교차 참조).
- **Stage 0** 자유 비행 카메라 → **A** 씬 그래프 코어 + 기존 씬 마이그레이션(픽셀 회귀) → **B** 전체 glTF
  계층 임포트(Lantern 3메시) → **C** 선언적 `.level` 포맷 + 로더/세이브 + 런타임 전환 → **D** 레벨 그래프
  + 카메라 기반 청크 스트리밍 → **E**(선택, Phase 12 이후) 쿡된 바이너리 레벨/월드.
- **완료 기준**: 계층 씬 그래프가 단일 드로우 리스트로 세 소비처(래스터/RT/컬)를 공급, glTF 계층이 올바른
  상대 트랜스폼으로 렌더, 선언적 레벨 핫스왑, 카메라 주행 시 청크 스트림 인/아웃(누수 없음), 두 백엔드
  픽셀 일치, 검증 클린.
- **범위 외**: glTF 애니메이션/스키닝(계층이 잠금 해제하는 후속이나 본 Phase 제외).
