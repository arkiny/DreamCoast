...
### B1-lite 구현 완료 (SCREEN_HIT 동반 자동, 2026-07-08)
`P_REFLECT_SCREEN_HIT=1`이면 **SSR/ssr_resolve 패스를 아예 기록하지 않고** composite의 SSR 블렌드를
하드 컷(`ssr_cut` push, pad2) — GDF 경로가 이미 per-ray 검증 스크린 색을 담으므로 무검증 SSR 블렌드는
중복 계상 + 자체 피드백 진동만 추가. 검증: 품질 동일, 프레임간 플리커 0.319→**0.275**(−14%) + SSR
패스 비용 절약. 잔여 wiggle = 트레이스 자체의 lit-history 피드백(temporal EMA가 감쇠) — 완전한 B1
(compaction 구조)로 추가 개선 여지. 갤러리 byte-identical, clippy/fmt clean.

### 완전 B1 wiggle 해소 구현 완료 (2026-07-08)
잔여 wiggle의 진짜 근본 2건을 격리·수정 (compaction은 불필요했음 — 성능 인에이블러로 B2에 남음):
1. **글로시 temporal 히스토리 전면 거부 버그**: `reflect_temporal`의 virtual-image 재투영 게이트가
   `sr == 0`이었는데 A1 resolve의 `spatial_off`가 **모든 픽셀의 sr을 0으로** 만들어 글로시까지 가상
   이미지를 재투영 — 글로시 로브의 hit distance는 프레임마다 수 m씩 요동하므로 저장된 가상점이
   매 프레임 disocclusion 반경(scene_radius×2%)을 초과 → **히스토리 len이 1에 고정, 글로시 표면은
   temporal 누적이 통째로 죽어 있었음**(A4b 공간 블러만 남음). 수정: 게이트를 실제 미러 임계
   `roughness < 0.125`로(0.125·8.0 = 2의 거듭제곱이라 spatial_off off 시 기존 `sr==0` 판정과
   bit-동등 → 갤러리/레거시 불변).
2. **EMA α 바닥 = AR(1) 정상과정**(warmup 100 vs 400 플리커 불변으로 측정 확인): 히스토리 캡 64가
   α를 1/64로 고정 — GGX/TAAU jitter + screen-hit lit-history 피드백이 영구 분산 바닥으로 남음.
   수정(호스트 전용): CONVERGE + **bit-정적 unjittered 카메라**(`view_proj_stable ==
   prev_view_proj_taau`)일 때 캡을 2^24로(패스의 `min(len+1, cap)`이 캡 아래에선 이미 running
   mean이므로 α=1/N 진짜 수렴) + 히스토리 클램프 off(수렴된 평균을 매 프레임 jitter된 3×3 통계로
   재클램프하면 방금 제거한 노이즈를 재주입). SSR resolve도 동일 처리(N→12 포화 해제+클램프 off).
   카메라 이동 시 `min(len, 64)`가 한 프레임에 카운터를 붕괴시켜 레거시 응답성 복귀.

**측정(glossyball 풀스택 SPP8+SCREEN_HIT+PREFILTER+GRID+CONVERGE32, 볼크롭 ±360, 연속 프레임쌍
평균):** 0.178(warmup 100↔400 불변 = 정상과정) → **0.065 @w100 → 0.050 @w400(감쇠 = 진짜 수렴)**.
볼이 씬 평균(풀프레임 0.073)보다 조용해짐. 글로시 볼 반사가 milky wash → 커튼/아치 구조 복원(누적이
로브 평균으로 수렴). chromeball 0.079 ≈ 씬 바닥. 갤러리 golden `af70c1a5` byte-identical PASS,
clippy/fmt/tests clean, 신규 패스 없음(비용 불변). 커밋 `be8b285`.

### 티어 통합 구현 완료 (2026-07-08, `6893f7d`)
`QualityPreset`에 신규 노브 7종(`reflect_stochastic`/`reflect_prefilter`/`reflect_glossy_spp`/
`reflect_screen_hit`/`cache_grid`/`card_mesh_capture`/`cache_adaptive_res`, 전부 `#[serde(default)]`
= OFF라 Low/Med/High·온디스크 RON 무변경). 소비자는 기존 `env.unwrap_or(base.<knob>)` 패턴 유지(명시
env 항상 우선), 갤러리는 `gallery_preset()`이 7종 전부 OFF 핀(구조적 앵커 보호, golden PASS).
**Apple 티어만 Metal-검증 스택을 기본 ON**(stochastic + prefilter 0.4 + SPP4 + screen-hit + 카드그리드
+ mesh capture; adaptive res는 relight +3ms라 DX≡VK 후 High 소재) — Apple 티어는 Metal 전용이므로
미검증 VK/D3D12 경로가 티어 기본으로 노출될 수 없음. 구조 변경: 티어/base 해석을 레벨 빌드 앞으로
호이스트(로드타임 캐시 노브가 필요), 카드그리드 env 게이트를 gdf.rs → `build_surface_cache` 호출자로
이동(티어 테이블 단일 소스), UI 티어 라이브스왑에 per-frame 4종 재파생 추가(로드타임 3종은 레벨 빌드
고정), 티어별 프리셋 단위테스트 잠금.
**측정(glossyball 2560×1440, 기본 env, fence-wait 중앙값 180프레임):** 45.9 → **30.0ms(−35%)** —
B1-lite SSR 패스 컷 + 그리드 + 프리필터 + 스크린히트 조기종료가 K=4 루프 비용을 상회. 볼 = 스파클
워시 → 구조 있는 글로시 반사, 볼크롭 플리커 0.054. 갤러리 golden PASS, clippy/fmt/tests(22) clean.
...
#### C2b 구현 완료 (opt-in `P11_CACHE_RES_FEEDBACK=<frame>`, `=1`→64; ADAPTIVE_RES 필요, 2026-07-08)
반사 콘 샘플러가 수락 카드마다 **풋프린트가 원한 pow2 res를 InterlockedMax**(호스트-visible u32/카드,
인덱스는 cache.y bits 8..15 동승) → 지정 프레임에 호스트가 리드백, 같은 텍셀 예산으로 재정규화
(`fuse::normalize_card_res`, assign에서 분리), **원샷 re-layout**(layout/아틀라스 재생성, wait_idle
후) → 재캡처 + 리셋 relight로 재수렴. 미읽힘 카드는 8² 플로어로 강등되어 반사가 실제 읽는 카드에
예산 집중 — C2a 정적 거리 프라이어의 원거리 카드 저해상을 해소.
**검증(glossyball HQ):** 초기 8..64/2.76M → 피드백 후 **16..64/1.68M**(61% 사용 — pow2 양자화의
불연속 스텝, 후속: 잔여 예산 greedy 승급). 재수렴 정상, 갤러리 byte-identical, clippy/fmt/tests clean.
**한계(정직):** 넓은 콘 로브(글로시 볼)는 mip 선택이 footprint 기준이라 mip0 res 승급에 둔감 —
승급의 시각 이득은 near-mirror/근거리 캐시 읽기에서 발현. 이 장면 지표는 인프라 검증 수준.
